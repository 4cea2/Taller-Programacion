...
BoundedQueueBlocking::BoundedQueueBlocking(const unsigned int max_size):
        max_size(max_size), closed(false) {}

bool BoundedQueueBlocking::try_push(const std::string& val) {
...
bool BoundedQueueBlocking::try_pop(std::string& val) {
    std::unique_lock<std::mutex> lck(mtx);
...
void BoundedQueueBlocking::push(const std::string& val) {
    std::unique_lock<std::mutex> lck(mtx);
...
std::string BoundedQueueBlocking::pop() {
    std::unique_lock<std::mutex> lck(mtx);
...
void BoundedQueueBlocking::close() {
    std::unique_lock<std::mutex> lck(mtx);
